...
// NEW: View Purchase Order (now opens in a modal for supplier POs)
...
// NEW: Load Pending Material Requests Table for Admin Alerts
function loadPendingMaterialRequestsTable() {
  const table = document.getElementById("pending-material-requests-table")
  if (!table) return
  ...
  fetch("/api/material_requests/list") // This endpoint already filters by role on backend
    ...
    .then((data) => {
      ...
      if (pendingRequests.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="7" style="text-align: center;">No hay solicitudes de material pendientes.</td></tr>'
        return
      }
      pendingRequests.forEach((request) => {
        const row = document.createElement("tr")
        // Only show actions for pending requests, which is what this table is for
        const actionsHtml = `
                          <button class="action-btn approve-material-request" data-id="${request.id}" title="Aprobar"><i class="fas fa-check"></i></button>
                          <button class="action-btn deny-material-request" data-id="${request.id}" title="Denegar"><i class="fas fa-times"></i></button>
                      `
      ...
      // Reuse the existing action setup for approve/deny
      setupAdminMaterialRequestActions()
    })
    .catch((error) => {
      console.error("Error al cargar solicitudes de material pendientes:", error)
      tbody.innerHTML = `<tr><td colspan="7" class="error-message">Error al cargar solicitudes de material pendientes: ${error.message}</td></tr>`
    })
}
