...
function showPage(pageId) {
  ...
  // Load data relevant to the active page
  if (pageId === "alertas") {
    loadAlertasDisenos()
  ...
  } else if (pageId === "material-requests") {
  ...
  if (confirm("¿Está seguro que desea eliminar este item?")) {
    fetch(`/api/inventario/delete/${itemId}`, {
      ...
      .catch((error) => {
        ...
        displayFlashMessage(`Error al eliminar el item: ${error.message}`, "error")
      })
  }
}
...
function loadAlertasDisenos() {
  ...
  fetch("/api/inventario/disenos")
    .then((response) => {
      console.log("DEBUG: Response for /api/inventario/disenos (alerts):", response)
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`)
      }
      return response.json()
    ...
    .then((data) => {
      console.log("DEBUG: Data for alertas disenos:", data)
      ...
      const disenosAgotados = disenosEnAlerta.filter((diseno) => diseno.m3_posibles === 0)
      const disenosLimitados = disenosEnAlerta.filter((diseno) => diseno.m3_posibles > 0)
      ...
      if (disenosAgotados.length > 0) {
        ...
        const disenosAgotadosNombres = disenosAgotados.map((diseno) => diseno.nombre).join(", ")
      ...
      if (disenosLimitados.length > 0) {
        ...
        const disenosLimitadosInfo = disenosLimitados
          .map((diseno) => `${diseno.nombre} (${diseno.m3_posibles}m³)`)
          .join(", ")
      ...
      const materialesLimitantes = disenosEnAlerta.flatMap((diseno) => diseno.limitante).filter(Boolean)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setupEventListeners() {
    ...
    if (addDesignBtn) addDesignBtn.addEventListener("click", () => openAddDesignModal())
    ...
    if (concreteDesignForm) {
      concreteDesignForm.addEventListener("submit", (event) => {
        ...
        fetch(apiUrl, {
          ...
          .then((response) => {
            console.log("DEBUG: Response for concrete design form submission:", response)
            if (response.ok) {
              return response.json()
            } else {
              return response.json().then((errorData) => {
                throw new Error(errorData.message || "Error al guardar diseño de concreto")
              })
            }
          })
          .then((data) => {
            displayFlashMessage(data.message, "success")
            designFormModal.style.display = "none" // Close modal
            showPage("inventario")
          })
          .catch((error) => {
            console.error("Error:", error)
            displayFlashMessage(`Error al guardar el diseño de concreto: ${error.message}`, "error")
          })
      })
    }
  ...
  // Initial data loads for the default active page (Alertas)
  showPage("alertas")
