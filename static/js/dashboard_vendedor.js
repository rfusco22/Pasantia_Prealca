...
const vendedoresDataMap = new Map() // Keep this map, but remove its usage in dispatch form
  ...
  return fetch("/api/concrete_designs") // Return the promise
    ...
    .then((data) => {
      ...
      itemRows.forEach((row) => {
        ...
        if (selectedDesign) {
          descriptionDisplay.value = `RC = ${selectedDesign.nombre} ADTVO SUPER PLASTIFICANTE WRDA79 FRACTIL 10%`
        } else {
  ...
  return fetch("/api/quotations/list") // Return the promise
    ...
    .then((data) => {
      ...
      data.forEach((quotation) => {
        const quotationDate = new Date(`${quotation.quotation_date}T00:00:00-04:00`) // Parse as local date
        const expiryDate = new Date(quotationDate)
        expiryDate.setDate(quotationDate.getDate() + quotation.validity_days)

        const row = document.createElement("tr")
        row.innerHTML = `
         <td>${quotation.quotation_number}</td>
         <td>${formatDate(quotation.quotation_date)}</td>
         <td>${formatDate(expiryDate.toISOString().split("T")[0])}</td>
...
// Function to add a new item row for quotations
...
<input type="text" class="quotation-item-unit-price px-3 py-2 border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-blue-300 text-sm shadow-sm" placeholder="P. Unitario" value="${
...
`
  ...
  const updateDescription = () => {
    ...
    if (selectedDesign && days) {
      descriptionDisplay.value = `RC = ${selectedDesign.nombre} ADTVO SUPER PLASTIFICANTE WRDA79 FRACTIL 10% A LOS ${days} DIAS` // Updated description
    } else {
      descriptionDisplay.value = ""
    }
    calculateTotals()
...
function generateQuotationPreview() {
  ...
  const itemRows = document.querySelectorAll("#quotation_items_container .quotation-item-row")
  itemRows.forEach((row) => {
    const code = row.querySelector(".quotation-item-code").value
    const description = row.querySelector(".quotation-item-description-display").value
    const days = row.querySelector(".quotation-item-days").value // Reintroduced
    const quantity = parseNumericInput(row.querySelector(".quotation-item-quantity").value)
    const unitPrice = parseNumericInput(row.querySelector(".quotation-item-unit-price").value)
    const itemTotal = quantity * unitPrice
...
function setupQuotationForm() {
  ...
  // Removed cotNotesInput as per user request
  //const cotSellerSelect = document.getElementById("cot_seller") // Get seller select

  if (!form) return

  // Ensure at least one item row is present initially, and concrete designs are loaded
  ...
  // Removed cotNotesInput event listener
  //cotSellerSelect.addEventListener("change", generateQuotationPreview) // Add event listener for seller select
  ...
  form.addEventListener("submit", (e) => {
    ...
    // Convert the displayed DD/MM/YYYY date to YYYY-MM-DD for backend submission
    const quotation_date = convertDDMMYYYYtoYYYYMMDD(cotDateInput.value)
    const client_id = cotClientSelect.value
    const seller_id = window.userInfo.vendedorDbId // Use the seller's DB ID from userInfo
    const validity_days = cotValidityInput.value
    const notes = "" // Notes field removed, send empty string or remove from backend if not needed
    ...
    fetch("/api/quotations", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          <div class="po-item-row header-row">
              <span style="width: 15%;">Código</span>
              <span style="width: 35%;">Descripción</span>
              <span style="width: 10%;">Días</span>
              <span style="width: 10%;">Cantidad</span>
              <span style="width: 15%; text-align: right;">P. Unitario</span>
              <span style="width: 10%; text-align: right;">Total</span>
...
function setupMainClienteFormVendedor() {
  ...
  // NEW: Use vendedorDbId if available, which comes from the 'vendedores' table
  if (vendedorIdInput && window.userInfo && window.userInfo.vendedorDbId) {
    vendedorIdInput.value = window.userInfo.vendedorDbId
  } else if (vendedorIdInput && window.userInfo && window.userInfo.id) {
    // Fallback to user.id if vendedorDbId is not set (e.g., if user is not a seller)
    // This case should ideally not happen if the user is a 'vendedor' and the API call succeeds
    vendedorIdInput.value = window.userInfo.id
  }

  // Ensure that if the user is a seller, their vendedorDbId is correctly set.
  // If not, disable the form and show a message.
  if (window.userInfo && window.userInfo.rol === "vendedor") {
    if (!window.userInfo.vendedorDbId) {
      console.error("[ERROR] VendedorDbId not found for logged-in seller. Disabling client registration form.")
      mainForm.querySelector('button[type="submit"]').disabled = true
      displayFlashMessage(
        "Su perfil de vendedor no está completamente configurado. No puede registrar clientes. Contacte a un administrador.",
        "error",
      )
      return // Exit the function to prevent further setup or submission
    } else {
      vendedorIdInput.value = window.userInfo.vendedorDbId
    }
  } else if (vendedorIdInput && window.userInfo && window.userInfo.id) {
    // Fallback for other roles or if vendedorDbId is not applicable/found for non-sellers
    vendedorIdInput.value = window.userInfo.id
  }
  ...
  const telefonoInput = document.getElementById("telefono_cliente_vendedor")
  ...
  direccionInput.addEventListener("input", () =>
    validateAddress(direccionInput, "direccion_cliente_vendedor_validation_message"),
  )
  telefonoInput.addEventListener("input", () =>
    validatePhone(telefonoInput, "telefono_cliente_vendedor_validation_message"),
  )
  documentoTypeInput.addEventListener("change", () =>
    validateVenezuelanDocument(
      documentoTypeInput.value,
      documentoNumberInput.value,
      documentoTypeInput.value !== "V", // isRif is true if type is not 'V' (Cédula)
      "documento_cliente_vendedor_validation_message",
    ),
  )
  documentoNumberInput.addEventListener("input", () =>
    validateVenezuelanDocument(
      documentoTypeInput.value,
      documentoNumberInput.value,
      documentoTypeInput.value !== "V",
      "documento_cliente_vendedor_validation_message",
    ),
  )
  ...
  mainForm.addEventListener("submit", (e) => {
    ...
    const isTelefonoValid = validatePhone(telefonoInput, "telefono_cliente_vendedor_validation_message")
    const isDocumentoValid = validateVenezuelanDocument(
      documentoTypeInput.value,
      documentoNumberInput.value,
      documentoTypeInput.value !== "V",
      "documento_cliente_vendedor_validation_message",
    ...
    if (!isNombreValid || !isDireccionValid || !isTelefonoValid || !isTelefonoValid || !isDocumentoValid) {
    ...
    fetch(mainForm.action, {
      ...
      .then((data) => {
        if (data.success) {
          ...
          setValidationFeedback(telefonoInput, null, "telefono_cliente_vendedor_validation_message")
          // Reset document validation feedback by re-running validation with empty values
          validateVenezuelanDocument(
            documentoTypeInput.value,
            "", // Empty number to clear feedback
            documentoTypeInput.value !== "V",
            "documento_cliente_vendedor_validation_message",
          )
...
async function fetchClientDataForEdit(clientId) {
  try {
    console.log(`[DEBUG] Attempting to get client data for ID: ${clientId}`)
    ...
    if (!contentType || !contentType.includes("application/json")) {
      ...
      console.error(
        "[ERROR] Expected JSON, but received non-JSON content. Content-Type:",
        contentType,
        "Response status:",
        response.status,
      ...
      displayFlashMessage(
        "Server error: Expected JSON response, but received invalid content. Check console for details.",
        "error",
      )
      editClientModal.style.display = "none"
      return // Exit function if content type is not JSON
    }
    ...
    try {
      ...
      const textResponse = await response.text()
      console.error(
        "[ERROR] Could not parse response as JSON. Error:",
        jsonError,
        "Text response (if available):",
        textResponse,
      )
      displayFlashMessage("Server error: Response is not valid JSON.", "error")
      editClientModal.style.display = "none"
      return
    }

    if (!response.ok) {
      console.error("[ERROR] Server responded with an error:", data)
      displayFlashMessage(`Server error loading client: ${data.message || response.statusText}`, "error")
      editClientModal.style.display = "none"
      return
    }
    ...
    if (data.documento && typeof data.documento === "string" && data.documento.includes("-")) {
      ...
      console.warn(
        "[WARN] 'documento' field is empty, null, or not in expected format (e.g., 'V-12345678') for client ID:",
        clientId,
        "Value:",
        data.documento,
      )
      editDocumentoTypeInput.value = "V"
      editDocumentoNumberInput.value = data.documento || ""
    }

    // Set the value of the hidden vendedor_id input
    const editVendedorClienteIdInput = document.getElementById("edit_vendedor_cliente_id") // NEW LINE
    if (editVendedorClienteIdInput) {
      // NEW LINE
      editVendedorClienteIdInput.value = data.vendedor_id // NEW LINE
    } // NEW LINE

    validateName(editNombreInput, "edit_nombre_cliente_validation_message")
    validateAddress(editDireccionInput, "edit_direccion_cliente_validation_message")
    validatePhone(editTelefonoInput, "edit_telefono_cliente_validation_message")
    validateVenezuelanDocument(
      editDocumentoTypeInput.value,
      editDocumentoNumberInput.value,
      editDocumentoTypeInput.value !== "V",
      "edit_documento_cliente_validation_message",
    )
  } catch (error) {
    console.error("[ERROR] Error getting client data for edit (network or parsing error):", error)
    displayFlashMessage(
      "Error al cargar datos del cliente para edición. Verifique la conexión o el formato de respuesta del servidor.",
      "error",
    )
    editClientModal.style.display = "none"
  }
...
// Add event listeners for validation on edit form inputs
editNombreInput.addEventListener("input", () => validateName(editNombreInput, "edit_nombre_cliente_validation_message"))
editDireccionInput.addEventListener("input", () =>
  validateAddress(editDireccionInput, "edit_direccion_cliente_validation_message"),
)
editTelefonoInput.addEventListener("input", () =>
  validatePhone(editTelefonoInput, "edit_telefono_cliente_validation_message"),
)
editDocumentoTypeInput.addEventListener("change", () =>
  validateVenezuelanDocument(
    editDocumentoTypeInput.value,
    editDocumentoNumberInput.value,
    editDocumentoTypeInput.value !== "V",
...
editDocumentoNumberInput.addEventListener("input", () =>
  validateVenezuelanDocument(
    editDocumentoTypeInput.value,
    editDocumentoNumberInput.value,
    editDocumentoTypeInput.value !== "V",
...
editClientForm.addEventListener("submit", async (e) => {
  ...
  const isTelefonoValid = validatePhone(editTelefonoInput, "edit_telefono_cliente_validation_message")
  const isDocumentoValid = validateVenezuelanDocument(
    editDocumentoTypeInput.value,
    editDocumentoNumberInput.value,
    editDocumentoTypeInput.value !== "V",
  ...
  // REMOVED: formData.delete("vendedor") // This line was removed to allow the seller to be updated

  try {
    const response = await fetch(`/api/clientes/${clientId}`, {
      method: "POST", // Flask uses POST for updates
      body: formData,
    })
    const data = await response.json()
    ...
    if (data.success) {
      ...
      await loadClientesForSelect() // Refresh data for quotation preview
      await loadClientesForDispatchSelect() // Refresh data for dispatch guide client select
      generateQuotationPreview() // Ensure quotation preview updates
      updateDispatchPreview() // Ensure dispatch guide preview updates
    } else {
      displayFlashMessage(data.message, "error")
    }
  } catch (error) {
...
async function deleteClient(clientId) {
  ...
  try {
    const response = await fetch(`/api/clientes/delete/${clientId}`, {
      ...
      body: JSON.stringify({ id: clientId }), // Send ID in body if needed by backend, though URL is primary
    })
    const data = await response.json()
...
async function loadInitialData() {
  ...
  // Después de que todos los datos iniciales estén cargados, se configuran los formularios.
  setTodayDateToInput("cot_date")
...
function renderQuotationDocument(data, containerId, isForPrinting = false) {
  ...
  const formatCurrency = (value) => {
    if (isNaN(value) || value === null) return "Bs.S 0,00"
  ...
  <div class="quotation-document">
      ...
      <div class="client-seller-details">
          <p><strong>Cliente:</strong> <span id="preview_client_name">${quotation.client_name || "SIN RELLENAR"}</span></p>
          <p><strong>Dirección:</strong> <span id="preview_client_address">${
            quotation.client_address || "SIN RELLENAR"
          }</span></p>
          <p><strong>Rif:</strong> <span id="preview_client_rif">${quotation.client_document || "SIN RELLENAR"}</span></p>
          <p><strong>Teléfono:</strong> <span id="preview_client_phone">${
            quotation.client_phone || "SIN RELLENAR"
          }</span></p>
          <p><strong>Vendedor:</strong> <span id="preview_seller_name">${
            quotation.seller_name || "SIN RELLENAR"
          }</span></p>
      </div>
...
function loadVendedores() {
  return fetch("/api/vendedores") // Return the promise
    ...
    .then((data) => {
      // The vendedor select is removed from the form, so no need to populate it.
      // Keep the map for potential other uses if needed.
...
// NEW: Cargar clientes para selects (for dispatch guide) - REVISED to use clientsDataMap
function loadClientesForDispatchSelect() {
  const clienteSelect = document.getElementById("cliente")
  if (clienteSelect) {
    clienteSelect.innerHTML = '<option value="">Seleccione cliente</option>'
    // Populate from the global map
    ...
    console.log("[DEBUG] 'cliente' select initial value:", clienteSelect.value)
...
function setupDespachoForm() {
  ...
  // Removed event listener for vendedor select as it's no longer in the HTML
  // document.getElementById("vendedor").addEventListener("change", updateDispatchPreview);
    ...
    fetch("/api/despachos", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          document.getElementById("preview_guia_number").textContent = data.guia || "N/A"
          document.getElementById("preview_hora_salida").textContent = data.hora_salida || "N/A"
...
function updateDispatchPreview() {
  ...
  // Added debug logs for all select elements and their values
  console.log("[DEBUG] fecha_hidden.value:", fecha)
  console.log("[DEBUG] m3.value:", m3)
  console.log("[DEBUG] diseno select value:", disenoSelect ? disenoSelect.value : "N/A")
  console.log("[DEBUG] cliente select value:", clienteSelect ? clienteSelect.value : "N/A")
  console.log("[DEBUG] chofer select value:", choferSelect ? choferSelect.value : "N/A")
  console.log("[DEBUG] camion select value:", camionSelect ? camionSelect.value : "N/A")
  // console.log("[DEBUG] vendedor select value:", vendedorSelect ? vendedorSelect.value : "N/A"); // Removed

  const selectedCliente = clientsDataMap.get(clienteSelect.value)
  const selectedChofer = choferesDataMap.get(choferSelect.value)
  const selectedCamion = camionesDataMap.get(camionSelect.value)
  // Removed selectedVendedor as it's no longer in the HTML
  // const selectedVendedor = vendedoresDataMap.get(vendedorSelect.value);
  const selectedDiseno = disenosDataMap.get(disenoSelect.value)

  // Added debug logs for the retrieved objects from maps
  console.log("[DEBUG] selectedCliente from clientsDataMap:", selectedCliente)
  console.log("[DEBUG] selectedChofer from choferesDataMap:", selectedChofer)
  console.log("[DEBUG] selectedCamion from camionesDataMap:", selectedCamion)
  // console.log("[DEBUG] selectedVendedor from vendedoresDataMap:", selectedVendedor); // Removed
  console.log("[DEBUG] selectedDiseno from disenosDataMap:", selectedDiseno)
  ...
  document.getElementById("preview_client_name").textContent = selectedCliente ? selectedCliente.nombre : "SIN RELLENAR"
  document.getElementById("preview_client_address").textContent = selectedCliente
    ? selectedCliente.direccion
    : "SIN RELLENAR"
  document.getElementById("preview_client_phone").textContent = selectedCliente
    ? selectedCliente.telefono
    : "SIN RELLENAR"
  document.getElementById("preview_client_document").textContent = selectedCliente
  ...
  document.getElementById("preview_camion_info").textContent = selectedCamion
    ...
    : "SIN RELLENAR"
  // Use the logged-in user's name for the Vendedor field
...
// NEW: Cargar tabla de registro de despachos para Admin (moved from admin, now for Vendedor)
function loadRegistroGuiaDespachoTable() {
  ...
  fetch("/api/despachos")
   ...
   <td>${despacho.guia}</td>
   <td>${despacho.m3}</td>
   <td>${despacho.concrete_design_name || "N/A"}</td>
   ...
   <td>
     ...
     <button class="action-btn print-dispatch-admin" data-id="${despacho.id}" title="Imprimir"><i class="fas fa-print"></i></button>
   </td>
 `
        tbody.appendChild(row)
      })
...
// NEW: Setup actions for dispatch guides in Admin table (moved from admin, now for Vendedor)
function setupDispatchActionsAdmin() {
  ...
  document.querySelectorAll(".print-dispatch-admin").forEach((button) => {
    button.addEventListener("click", function () {
      const dispatchId = this.getAttribute("data-id")
      printDispatchGuide(dispatchId)
    })
  })
}

// NEW: Function to view dispatch details in the modal (moved from admin)
...
// NEW: Function to print dispatch guide
async function printDispatchGuide(dispatchId) {
  try {
    const response = await fetch(`/api/despachos/${dispatchId}`)
    const data = await response.json()
    ...
    const printableContent = `
      ...
      <head>
          ...
          <title>Guía de Despacho ${data.guia}</title>
          <style>
              ...
              .dispatch-details p { margin: 0; font-size: 0.9em; }
              .received-by-section { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 0.9em; }
              @media print {
                  body { margin: 0; }
      ...
      <body>
          <div class="container">
              ...
              <h4>GUÍA DE DESPACHO: ${data.guia || "N/A"}</h4>
              ...
              <div class="dispatch-details" style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 20px; font-size: 0.9em;">
                  <p style="margin: 0;"><strong>Cliente:</strong> ${data.cliente_nombre || "N/A"}</p>
                  <p style="margin: 0;"><strong>Dirección:</strong> ${data.cliente_direccion || "N/A"}</p>
                  <p style="margin: 0;"><strong>Teléfono:</strong> ${data.cliente_telefono || "N/A"}</p>
                  <p style="margin: 0;"><strong>RIF/Cédula:</strong> ${data.cliente_documento || "N/A"}</p>
                  <p style="margin: 0;"><strong>Chofer:</strong> ${data.chofer_nombre || "N/A"}</p>
                  <p style="margin: 0;"><strong>Vendedor:</strong> ${data.vendedor_nombre || "N/A"}</p>
                  <p style="margin: 0;"><strong>Camión:</strong> ${data.camion_placa || "N/A"} - ${data.camion_modelo || "N/A"}</p>
                  <p style="margin: 0;"><strong>M3:</strong> ${data.m3 || "N/A"}</p>
              ...
              <div class="received-by-section">
                  <p style="margin: 0;"><strong>RECIBIDO POR:</strong></p>
                  <p style="margin: 0;"><strong>Firma:</strong> _________________________</p>
                  <p style="margin: 0;"><strong>Nombre:</strong> ${data.received_by || "_________________________"}</p>
                  <p style="margin: 0;"><strong>Fecha y Hora:</strong> ${data.received_by && data.hora_llegada ? `${formatDate(data.fecha)} ${data.hora_llegada}` : "_________________________"}</p>
              </div>
          </div>
      </body>
      </html>
...
// NEW: Setup Purchase Order Details Modal (moved from admin, now for dispatch details)
...
// Call setupPurchaseOrderDetailsModal on DOMContentLoaded
document.addEventListener("DOMContentLoaded", () => {
  // ... existing calls ...
  setupPurchaseOrderDetailsModal() // NEW
})
  ...
  async function loadUserInfo() {
    try {
      ...
      if (users.success === false) {
        displayFlashMessage(users.message, "error")
        return
      }
      ...
      if (currentUser) {
        if (userPhotoElement) {
          userPhotoElement.src = currentUser.foto || "/static/img/user.png"
          userPhotoElement.alt = currentUser.nombre
        }
        if (userNameElement) {
        ...
        displayFlashMessage("User data not found.", "error")
      }
    } catch (error) {
      console.error("Error loading user info:", error)
      displayFlashMessage("Error loading user information.", "error")
    }
  }

  // Load clients for the "Clientes" section
  async function loadClients() {
    const clientsTableBody = document.getElementById("clientsTableBody")
    if (!clientsTableBody) return

    clientsTableBody.innerHTML = '<tr><td colspan="6" class="text-center">Cargando clientes...</td></tr>'
    ...
    try {
      ...
      if (clients.error) {
        displayFlashMessage(clients.error, "error")
        clientsTableBody.innerHTML = `<tr><td colspan="6" class="text-center text-danger">${clients.message || "Error al cargar clientes."}</td></tr>`
        return
      }
      ...
      clients.forEach((client) => {
        ...
        row.innerHTML = `
              ...
              <td>
                  <button class="btn btn-sm btn-info edit-client-btn" data-id="${client.id}" title="Editar">
                      <i class="fas fa-edit"></i>
                  </button>
                  <button class="btn btn-sm btn-danger delete-client-btn" data-id="${client.id}" title="Eliminar">
      ...
      clientsTableBody.querySelectorAll(".edit-client-btn").forEach((button) => {
        button.addEventListener("click", function () {
          ...
          // Implement edit client logic (e.g., open modal with client data)
          console.log("Edit client:", clientId)
          // Example: fetch client data and populate a form
          fetch(`/api/clientes/${clientId}`)
            ...
            .then((data) => {
              ...
              displayFlashMessage(`Cargando datos para editar cliente ${data.nombre}`, "info")
            })
            .catch((error) => {
              console.error("Error fetching client for edit:", error)
              displayFlashMessage("Error al cargar datos del cliente para edición.", "error")
            })
        })
      })

      // Add event listeners for view dispatch button
      dispatchGuideTableBody.querySelectorAll(".view-dispatch-btn").forEach((button) => {
        button.addEventListener("click", function () {
          const dispatchId = this.dataset.id
          console.log("View dispatch:", dispatchId)
          // Implement view dispatch logic (e.g., open modal with dispatch data)
          fetch(`/api/despachos/${dispatchId}`)
            .then((res) => res.json())
            .then((data) => {
              if (data.error) {
              ...
              displayFlashMessage(`Cargando detalles de guía ${data.guia}`, "info")
            })
            .catch((error) => {
              console.error("Error fetching dispatch details:", error)
              displayFlashMessage("Error al cargar detalles de la guía de despacho.", "error")
            })
        })
      })
    } catch (error) {
      console.error("Error loading dispatch guides:", error)
      dispatchGuideTableBody.innerHTML = `<tr><td colspan="9" class="text-center text-danger">Error al cargar guías de despacho.</td></tr>`
      displayFlashMessage("Error al cargar guías de despacho.", "error")
    }
  }

  // Initial load of user info, clients, and dispatch guides
  loadUserInfo()
  loadClients()
  loadDispatchGuides()

  // Expose displayFlashMessage globally if needed by other parts of the application
  window.displayFlashMessage = displayFlashMessage

  // Load dispatch guides for the "Registro de Guías de Despacho" section
  async function loadDispatchGuides() {
    const dispatchGuideTableBody = document.getElementById("dispatchGuideTableBody")
    if (!dispatchGuideTableBody) return

    dispatchGuideTableBody.innerHTML = '<tr><td colspan="9" class="text-center">Cargando guías de despacho...</td></tr>'

    try {
      const response = await fetch("/api/despachos")
      const dispatchGuides = await response.json()

      if (dispatchGuides.error) {
        displayFlashMessage(dispatchGuides.error, "error")
        dispatchGuideTableBody.innerHTML = `<tr><td colspan="9" class="text-center text-danger">${dispatchGuides.message || "Error al cargar guías de despacho."}</td></tr>`
        return
      }

      dispatchGuideTableBody.innerHTML = "" // Clear loading message

      if (dispatchGuides.length === 0) {
        dispatchGuideTableBody.innerHTML =
          '<tr><td colspan="9" class="text-center">No hay guías de despacho registradas.</td></tr>'
        return
      }

      dispatchGuides.forEach((guide) => {
        const row = dispatchGuideTableBody.insertRow()
        const disenoNombre = guide.diseno_resistencia
          ? `${guide.diseno_resistencia} kgf/cm² - ${guide.diseno_asentamiento}"`
          : "N/A"
        ...
        row.innerHTML = `
               ...
               <td>${guide.guia}</td>
               <td>${guide.m3}</td>
               <td>${guide.concrete_design_name || "N/A"}</td>
               <td>${guide.cliente_nombre || "N/A"}</td>
               <td>${guide.chofer_nombre || "N/A"}</td>
               <td>${guide.camion_placa || "N/A"}</td>
               <td>${guide.vendedor_nombre || "N/A"}</td>
               <td>${guide.status}</td>
               <td>
                   <button class="btn btn-sm btn-info view-dispatch-btn" data-id="${guide.id}" title="Ver Detalles">
                       <i class="fas fa-eye"></i>
                   </button>
                   <!-- Add other actions like edit/delete if applicable for this role -->
               </td>
           `
      })
